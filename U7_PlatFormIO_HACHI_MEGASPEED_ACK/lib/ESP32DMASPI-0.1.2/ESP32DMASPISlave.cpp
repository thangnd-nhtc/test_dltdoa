...
bool Slave::wait(uint8_t* rx_buf, const uint8_t* tx_buf, const size_t size) {
    ...
    esp_err_t e = spi_slave_transmit(host, &transactions.back(), timeout);
    if (e != ESP_OK) {
...
bool Slave::queue(uint8_t* rx_buf, const uint8_t* tx_buf, const size_t size) {
    ...
    addTransaction(rx_buf, tx_buf, size);
    esp_err_t e = spi_slave_queue_trans(host, &transactions.back(), timeout);
...
void Slave::yield() {
    ...
    for (uint8_t i = 0; i < n; ++i) {
        ...
        esp_err_t e = spi_slave_get_trans_result(host, &r_trans, timeout);
        if (e != ESP_OK)
        {
...
void Slave::setTimeout(uint32_t time){
    timeout = time;
}
